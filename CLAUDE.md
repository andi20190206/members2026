...
- PR 合并完成后,**主动切回默认分支并 pull 最新代码**,为下一轮任务做好准备。
- **合并目标**: 一律以仓库**当前默认分支**(GitHub Pages 部署源)为 PR base,
  目前是 `claude/membership-system-XVNXu`。用户说"合并 / 发布"时直接执行,无需追问目标。
  默认分支由用户在 GitHub Settings 维护;如有变更,请用户告知后同步本文件。
